...
All six transactional types include meaningful plaintext and semantic HTML generated from the same content; React escapes dynamic names/titles and CTA URLs accept only HTTP(S) without credentials. Keep subjects stable and business triggers unchanged.

| Actual transactional type | Entry point | CTA |
|---|---|---|
| Admin enrollment request (action needed) | `createJoinRequest` → `notifyJoinRequestCreated` (onboarding and student API) | `/app/parish-admin/join-requests` |
...
Admin request notifications contain only the student display name, course title, parish name, and review action. Current `parish_memberships` rows with role `parish_admin` define active access; revocation deletes the membership (there is no separate status column). Recipient emails come from `user_profiles`, are deduplicated by account and normalized mailbox, and are never included in message content. The shared insertion boundary invokes one notification per newly created request, after persistence, with a request-specific provider idempotency key. Duplicate/pending requests and failed inserts do not notify. Notification failure cannot undo request creation and logs only a fixed safe message.
